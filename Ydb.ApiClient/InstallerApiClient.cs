...
using Castle.Core;
using Ydb.ApiClient.Intrastructure.YdbNhibernate.Repository;
using Ydb.ApiClient.Application;
using Ydb.ApiClient.Intrastructure.YdbNhibernate.UnitOfWork;

namespace Ydb.ApiClient
{
    public class InstallerApiClient : IWindsorInstaller
    {
        FluentConfiguration dbConfigInstantMessage;
        public InstallerApiClient(FluentConfiguration dbConfigInstantMessage)
        {
        ...
        {
            container.Register(Classes.FromThisAssembly().InSameNamespaceAs<RepositoryClient>()
        ...
        {
            container.Register(Classes.FromThisAssembly().InSameNamespaceAs<ClientService>()
                                .WithService.DefaultInterfaces()
                );
        ...
        const string SesssionFactoryName = "ApiClientSessionFactory";
        ...
        private void InstallDb(IWindsorContainer container, IConfigurationStore store)
        {
            var _sessionFactory =
                ...
                dbConfigInstantMessage
                    .Mappings(m => m.FluentMappings.AddFromAssemblyOf<Ydb.ApiClient.Infrastructure.YdbNhibernate.Mapping.ClientMap>())
