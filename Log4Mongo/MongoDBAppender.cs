...
namespace Log4Mongo
{
	public class MongoDBAppender : AppenderSkeleton
	{
		...
		private BsonDocument BuildBsonDocument(LoggingEvent log)
		{
            ...
            doc.Add("timestamp", log.TimeStamp);
            string[] strApiCount = { "","",""};
			foreach (MongoAppenderFileld field in _fields)
			{
                object value = field.Layout.Format(log);
                if (field.Name == "logger2" && value.ToString ()== "Rule.v1.RestfulApi.Web.Dianzhu")
                {
                    strApiCount[0] = "1";
                }
                if (field.Name == "logger")
                {
                    strApiCount[1] = value.ToString().Replace("Ydb.", "").Replace(".Rule.v1.RestfulApi.Web.Dianzhu", "");
                }
                if (field.Name == "message")
                {
                    strApiCount[2] = value.ToString();
                }
                //BsonValue bsonValue;
            ...
            if (strApiCount[0] == "1")
            {
                UpdateApiCount(strApiCount[1], strApiCount[2]);
            }
			return doc;
		}

        private void UpdateApiCount(string apiRoute, string message)
        {

            var collection = GetCollection("ApiInfos");
            ...
            var collection1 = GetCollection("RequestInfos");
            var doc = new BsonDocument();
