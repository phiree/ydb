...
namespace Ydb.BusinessResource.Application
{
    public class DZServiceService : IDZServiceService
    {
        ...
        public ActionResult<ServiceOpenTimeForDay> ModifyWorkTimeDay(string serviceId,DayOfWeek dayOfWeek, string workTimeId,string timeBegin,string endtime,int maxOrder)
        {
            ...
            ServiceOpenTimeForDay sotf= service.ModifyWorkTime(dayOfWeek, maxOrder, workTime.TimePeriod, new TimePeriod(new Time(timeBegin), new Time(endtime)),out errMsg);
            result.ResultObject = sotf;
            return result;
        }
