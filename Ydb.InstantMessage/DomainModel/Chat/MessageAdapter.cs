...
namespace Ydb.InstantMessage.DomainModel.Chat
{
    ...
    {
        private log4net.ILog log = log4net.LogManager.GetLogger("Ydb.InstantMessage.DomainModel.Chat.MessageAdapter");
        ...
        public Message ChatToMessage(ReceptionChat chat, string server)
        {
            ...
            //如果是发广播, 则 toId里面已经包含了@符号.
            string toId;
            if (chat.ToId.Contains("@"))
            {
                toId = chat.ToId + ".";
            }
            else
            ...
            msg.To = new agsXMPP.Jid(toId + server + "/" + chat.ToResource);//发送对象
